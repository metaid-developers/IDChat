...
const trySendImage = async () => {
  ...
  const messageDto = {
    ...
    channelId:talk.activeSubChannel?.channelId,
    groupId:talk.activeSubChannel?.groupId,
    userName: userStore.last?.name!,
    attachments,
    content: '',
    originalFileUrl,
    channelType:ChannelType.Broadcast, //talk.activeChannelType as ChannelType,
    reply: props.quote,
  }
...
const trySendText = async (e: any) => {
  ...
  if (talk.activeChannel?.groupId) {
    content = encrypt(chatInput.value, talk.activeSubChannel.channelId.substring(0, 16))
  } 
  ...
  const messageDto = {
    ...
    groupId:talk.activeChannel.groupId,
    channelId: talk.activeSubChannel.channelId,
    userName: userStore.last?.name || '',
